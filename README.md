...
Bird Companion counts typing events but does not store typed text, typed order, window titles, or app names. Persistent local storage keeps aggregate counts only: total keys, per-key counts, per-day counts, language, bird style, and sound settings.
...
GitHub releases use semantic tags such as `v0.2.0`. Current source version is tracked in `package.json`.
...
Bird Companion 只统计按键事件，不保存具体输入文本、输入顺序、窗口标题或应用名称。本地持久化只保存聚合数据：累计按键、按键分布、每日计数、语言、小鸟形象和声音设置。
...
GitHub release 使用 `v0.2.0` 这样的语义化 tag。当前源码版本记录在 `package.json`。
