...
2. Download `Bird Companion Setup 0.2.1.exe`.
...
- Press the small key-detail button beside the `keys` badge to open the compact typing detail card.
...
`npm test` verifies the sound engine, global key translation, mascot style state, i18n copy, settings, window orientation, and typing stats. `npm run smoke` opens Electron in smoke-test mode and verifies the renderer boots successfully. `npm run dist:win` builds the Windows installer in `dist/`.
...
GitHub releases use semantic tags such as `v0.2.1`. Current source version is tracked in `package.json`.
...
2. 下载 `Bird Companion Setup 0.2.1.exe`。
...
`npm test` 会检查声音引擎、全局按键转换、小鸟形象状态、国际化文案、设置、窗口朝向和打字统计。`npm run smoke` 会以 smoke-test 模式打开 Electron，确认渲染进程可以正常启动。`npm run dist:win` 会在 `dist/` 里生成 Windows 安装包。
...
GitHub release 使用 `v0.2.1` 这样的语义化 tag。当前源码版本记录在 `package.json`。
