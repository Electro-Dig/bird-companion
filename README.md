...
Bird Companion is a small always-on-top desktop bird that reacts to your typing rhythm with bird calls, motion, and lightweight typing stats. It is an Electron desktop pet, not a browser page, so it can sit on your desktop, be dragged around, and optionally listen for global keyboard activity.

### Highlights

- Transparent, frameless, always-on-top desktop pet window.
- Bird calls driven by typing rhythm.
- Feedback frequency modes: `1`, `3`, or `5` keys per bird-call response.
- Mute, volume, and compact feedback controls.
- Persistent total key counter, daily stats, and per-key aggregate counts.
- Optional global keyboard listening for cross-app typing feedback.
- Automatic facing direction based on screen position.
- English/Chinese interface toggle. English is the default.
- Xeno-canto source links for the currently playing bird call.
...
2. Download `Bird.Companion.Setup.0.2.4.exe`.
...
3. Open `dist/mac-arm64/Bird Companion.app` directly, or unzip `dist/Bird.Companion.mac-arm64.0.2.4.zip`.

Other channels:

- Source run: clone the repo and run `npm install && npm start`.
...
- Portable build: maintainers can run `npm run dist:win:portable` to create `Bird.Companion.Portable.0.2.4.exe`.
- Zip build: maintainers can run `npm run dist:win:zip` to create `Bird.Companion.0.2.4.zip`.

Package-size note: Bird Companion is currently Electron-based, so every desktop build includes Chromium and Node. `v0.2.4` keeps only English/Chinese Electron locales and excludes unused spectrogram PNGs from packaged builds. A substantially smaller build would likely require a future Tauri or WebView2 rewrite.
...
- Press the `keys` badge or its detail button to open the compact typing detail card.
- Press `More` on the current sound card to open the original xeno-canto recording page.
...
```text
bird-companion/
  build/                 Packaging assets and pack hooks
  data/                  Bird-call metadata and bundled audio assets
...
- Keep global listening visibly controllable.
...
`npm test` verifies the sound engine, global key translation, mascot style state, i18n copy, settings, window orientation, and typing stats. `npm run smoke` opens Electron in smoke-test mode and verifies that the renderer boots successfully. `npm run dist:mac:zip` builds an unsigned Apple Silicon macOS zip for local testing in `dist/`. `npm run dist:mac` builds both macOS `dmg` and `zip` targets. `npm run dist:win` builds the Windows installer in `dist/`.
...
- smaller future runtime with Tauri or WebView2.

### Releases

GitHub releases use semantic tags such as `v0.2.4`. Current source version is tracked in `package.json`.
...
Bird Companion 是一个常驻桌面的小鸟宠物。它会根据键盘输入节奏发出鸟叫、做出轻量动画反馈，并记录简洁的打字统计。它不是浏览器页面，而是一个 Electron 桌面应用，所以可以悬浮在桌面上、自由拖动，也可以选择开启全局键盘监听。
...
- 支持每 `1`、`3`、`5` 次按键触发一次鸟叫。
- 支持静音、音量和紧凑反馈控制。
- 持久化累计按键数、每日统计和按键分布统计。
- 可选全局键盘监听，用于其他软件中的打字反馈。
- 根据屏幕位置自动调整小鸟朝向。
- 支持中英文界面切换，默认英文。
- 当前鸟叫卡片可跳转到对应 xeno-canto 原始录音页面。
...
2. 下载 `Bird.Companion.Setup.0.2.4.exe`。
...
3. 直接打开 `dist/mac-arm64/Bird Companion.app`，或解压 `dist/Bird.Companion.mac-arm64.0.2.4.zip` 后运行。

其他方式：

- 源码运行：克隆仓库后执行 `npm install && npm start`。
- Mac 测试构建：维护者可以执行 `npm run dist:mac`，为 Apple Silicon Mac 生成未签名的本地测试 `dmg` 和 `zip`。
- 便携版：维护者可以执行 `npm run dist:win:portable`，生成 `Bird.Companion.Portable.0.2.4.exe`。
- Zip 版：维护者可以执行 `npm run dist:win:zip`，生成 `Bird.Companion.0.2.4.zip`。

体积说明：Bird Companion 目前基于 Electron，所以桌面构建会自带 Chromium 和 Node。`v0.2.4` 只保留英文/中文 Electron 语言包，并从打包产物中排除了当前桌面宠物界面未使用的频谱图 PNG。若要显著缩小体积，后续大概率需要改为 Tauri 或 WebView2 版本。
...
`npm test` 会验证声音引擎、全局按键转换、小鸟形象状态、i18n 文案、设置、窗口朝向和打字统计。`npm run smoke` 会用 smoke-test 模式打开 Electron 并验证渲染进程可正常启动。`npm run dist:mac:zip` 会在 `dist/` 里生成未签名的 Apple Silicon macOS 本机测试 zip。`npm run dist:mac` 会同时生成 macOS `dmg` 和 `zip`。`npm run dist:win` 会生成 Windows 安装包。

### 路线图

更多方向见 [docs/ROADMAP.md](docs/ROADMAP.md)。简要包括：

- 更丰富的小鸟动作系统：扇翅、张嘴鸣叫、跳跃、睡眠、注意力和 idle loop；
- 每日打字摘要和重置控制；
- 更多声音性格和鸟叫库选择；
- 基于打字节奏的番茄钟/专注模式；
- 基于更高质量 sprite 或 vector rig 的 mascot skin 系统；
- 未来用 Tauri 或 WebView2 缩小运行时体积。

### 发布

GitHub release 使用 `v0.2.4` 这样的语义化 tag。当前源码版本记录在 `package.json`。
...
内置鸟叫来自 `data/samples-data.js` 中列出的 xeno-canto 录音。每条样本都保留了 xeno-canto ID、来源链接、录音者、许可证和录音元数据。当某个声音被触发时，桌面卡片上的 `更多` 按钮会打开对应的 xeno-canto 原始录音页面。多数内置录音使用 Creative Commons BY-NC-SA 4.0，因此声音资源仅适合非商业用途，除非原录音者另行授权。
